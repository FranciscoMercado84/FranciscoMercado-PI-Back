...
```
franciscomercado-pi-back.onrender.com
```
...
**Health Check**:
```bash
curl https://franciscomercado-pi-back.onrender.com/health
```
...
```
https://franciscomercado-pi-back.onrender.com/api-docs
```
...
```bash
curl -X POST https://franciscomercado-pi-back.onrender.com/api/v1/auth/register \
...
**Login**:
```bash
curl -X POST https://franciscomercado-pi-back.onrender.com/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "cliente@test.com",
    "password": "Test1234!"
